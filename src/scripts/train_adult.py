...
for model_id in range(30):
    ...
    proto_model = ProtoModel(output_sizes, input_size=input_size, decode_weight=0, classification_weights=classification_weight,
                             ...
                             disentangle_weights=disentangle_weights, kl_losses=kl_losses)
    proto_model.train(train_data, train_data, train_outputs_one_hot, batch_size=128, epochs=num_epochs)
    y_accs, s_diff, _, _, disparate_impact, demographic = proto_model.evaluate(test_data, test_data, test_outputs, test_outputs_one_hot, plot=False, do_fairness_eval=True)
    y_acc = y_accs[0]
