...
def plot_latent_prompt(encoding, labels=None, test_encodings=None, block=True, ax=None, classes=None, savepath=None, show=True):
    ...
    if ax is None:
        ...
        # ax.set_title("German Encodings", fontsize=20)
        ax.set_title("Fashion Encodings", fontsize=20)
    ...
    if test_encodings is not None:
        for i, enc in enumerate(test_encodings):
            marker = 'x' if i < 10 else 'o'
            plot_gray_encoding(np.reshape(enc, (1, -1)), ax, pca, marker=marker)
...
def plot_gray_encoding(encoding, ax, pca, marker='x'):
    ...
    if new_base_fig:
        fig, ax = plt.subplots()
    ax.scatter(x, y, s=200, marker=marker, c='black')
