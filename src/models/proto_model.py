...
from data_parsing.mnist_data import batch_elastic_transform
from models.dist_prob_layer import DistProbLayer
from models.proto_layer import ProtoLayer
from utils.plotting import plot_latent_prompt, plot_rows_of_images, plot_mst
...
class ProtoModel:
    def __init__(self, output_sizes, duplication_factors=None, input_size=784, decode_weight=1,
        ...
        network_configs = {'dense_mnist': (self.build_network_parts, self.build_standard_dec, keras.optimizers.Adam(), []),
        ...
        self.proto_layers, self.classifier_layers, self.label_layers, self.dist_enc, self.encoder = self.enc_fn(output_sizes)
        self.build_overall_network()
        # self.print_proto_bases()
    ...
    def build_standard_dec(self, feature_enc, dec_input):
    ...
    def build_cifar_conv_dec(self, feature_enc, dec_input):
        ...
        d3 = layers.Conv2DTranspose(32, (3, 3), strides=(2, 2), padding='same', output_padding=(1, 1),
                                    activation='relu')
        d4 = layers.Conv2DTranspose(32, (3, 3), strides=(2, 2), output_padding=(1, 1), padding='same', activation='relu')
        d5 = layers.Conv2DTranspose(3, (3, 3), strides=(2, 2),  output_padding=(1, 1), padding='same', activation='sigmoid')
        d6 = layers.Reshape((self.input_size,), name='finalreshape')
        d = d1(feature_enc)
        d = d2(d)
        d = d3(d)
    ...
    def build_overall_network(self):
        ...
        for i, proto_layer in enumerate(self.proto_layers):
            ...
            classification_pred = self.classifier_layers[i]([dist_to_protos, self.label_mask_layers[i]])
            self.predictors.append(keras.Model([new_input, self.label_mask_layers[i]], classification_pred, name='predictor' + str(i)))
        self.projectors = []
        for i, proto_layer in enumerate(self.proto_layers):
        ...
        #         print("Alignment at start", keras.backend.eval(align))
        ...
        # Messing around with custom callback, but it doesn't work.
        class CustomCallback(keras.callbacks.Callback):
            def on_epoch_end(self, epoch, logs=None):
                my_metric = epoch
                tf.summary.scalar('learning rate', data=my_metric, step=epoch)
                # keys = list(logs.keys())
    ...
    def train_on_batches(self, inputs, outputs, batch_size=64, epochs=10, input_height=28):
        ...
        for epoch in range(epochs):
            ...
            for batch_idx in range(batches_per_epoch):
                ...
                elastic_batch_x = batch_elastic_transform(batch_data_inputs, sigma=sigma, alpha=alpha, height=input_height, width=input_width)
    ...
    # To do the hierarchical training technique and having finer-grained classification conditioned on the higher-
    # level classes, need to break up training into separate groups.
    # def train_hierarchical(self, inputs, outputs, masks, batch_size=32, epochs=10):
    #     for epoch in range(epochs):
    ...
    #         permuted_inputs = [inputs[i][permutation_idxs[i]] for i in range(len(inputs))]
    #         permuted_outputs = [outputs[i][permutation_idxs[i]] for i in range(len(outputs))]
    #         permuted_masks = [masks[i][permutation_idxs[i]] for i in range(len(masks))]
    #         batches_per_epoch = min([permutation_idxs[i].shape[0] // batch_size for i in range(len(inputs))])
    #         for batch_idx in range(batches_per_epoch):
    #             print("Batch", batch_idx, "of", batches_per_epoch)
    #             for classification_task in range(len(inputs)):
    #                 # print("Classification idx", classification_task, "of 21 (for cifar)")
    #                 full_idx = batch_idx * batch_size
    #                 batch_data_inputs = permuted_inputs[classification_task][full_idx: (batch_idx + 1) * batch_size]
    #                 batch_data_outputs = permuted_outputs[classification_task][full_idx: (batch_idx + 1) * batch_size]
    #                 batch_mask = permuted_masks[classification_task][full_idx: (batch_idx + 1) * batch_size]
    #                 # training_inputs = [batch_data_inputs, batch_data_outputs]
    #                 training_inputs = [batch_data_inputs, batch_data_outputs, batch_mask]
    #                 # Insert masking
    #                 self.all_models[classification_task].train_on_batch(training_inputs)
    ...
    def evaluate_hierarchical(self, x, targ_x, ys, y_one_hots, masks, coarse_to_fine, plot=True, gold_tree=None):
        ...
        #     predictions = model.predict([inputs[i], outputs_one_hot[i], masks[i]])
        #     masked_prediction = np.multiply(predictions, masks[i])
        #     num_correct = 0
        #     for j, prediction in enumerate(masked_prediction):  # FIXME: restrict to just the section allowed by masks.
        #         if np.argmax(prediction) == outputs[i][j]:
        #             num_correct += 1
    ...
    def evaluate(self, x, targ_x, ys, y_one_hots, masks=None, plot=True, protected_idx=1, do_fairness_eval=False, gold_tree=None):
        proto_weights = self.proto_layers[0].get_weights()[0]
        all_inputs = [x, targ_x]
        all_inputs.extend(y_one_hots)
        if masks is None:
        ...
        # self.plot_protos(proto_weights, plot=plot)
        # self.plot_recons(predictions, x, plot)
        alignments = self.get_alignment()
        # prob_updates = self.get_grad_orthogonality(x, ys)
        s_diff = None
        ...
        if do_fairness_eval:
            ...
            s_diff = self.predict_protected(encodings, ys, protected_idx=protected_idx)
            disparate_impact, demographic = self.get_disparity_metric(predictions, ys, protected_idx=protected_idx)
        average_cost = -1
        if gold_tree is not None:
            average_cost = self.get_confusion_matrix(ys, numerical_preds, true_tree=gold_tree)
            print("Average cost", average_cost)
        return y_accs, s_diff, alignments, prob_updates, disparate_impact, demographic, average_cost
        ...
        for i, coarse_prediction in enumerate(predictions[1]):
            ...
            for possible_coarse in coarse_to_fine.keys():
                ...
                for fine_idx in coarse_to_fine.get(possible_coarse):
                    joint_prediction[fine_idx] = normalized_masked[fine_idx] * coarse_prediction[fine_to_coarse.get(fine_idx)]
    ...
    def eval_corrected(self, x, ys):
        ...
        for target_classification in range(self.output_sizes[0]):
            ...
            for pred_idx, prediction in enumerate(predictions):
                ...
                grads = self.get_grads(tf.reshape(var_enc, (1, -1)), tf.reshape(tf.one_hot(target_classification, self.output_sizes[0]), (1, -1)), self.predictors[0])
                num_steps = 10
                ...
                plot_rows_of_images([decodings], savepath=img_save_location + 'corrected_to_' + str(target_classification), show=False)
                ...
                for step in range(num_steps):
                    ...
                    decodings[step + 1] = new_dec
                img_save_location = '../saved_models/'
                plot_rows_of_images([decodings], savepath=img_save_location + 'non_grad_corrected_to_' + str(target_classification), show=False)
    ...
    def get_grad_orthogonality(self, x, ys):
        prob_updates = []
        other_updates = []
        encodings = self.encoder.predict(x)
        for predictor_idx, predictor in enumerate(self.predictors):
            # Add dummy mask if you want to do this.
            curr_predictions = predictor.predict(encodings)
            for target_classification in range(self.output_sizes[predictor_idx]):
                for magnitude in [1.0]:
                    ...
                    for i, test_prediction in enumerate(curr_predictions):
                        ...
                        var_enc = tf.constant(np.reshape(encoding, (1, -1)))
                        grads = self.get_grads(tf.reshape(var_enc, (1, -1)), tf.reshape(tf.one_hot(correct_class, self.output_sizes[predictor_idx]), (1, -1)), predictor)
                        confidence = predictor.predict(np.reshape(encoding, (1, -1)))
                        new_enc = np.reshape(encoding - magnitude * keras.backend.eval(grads), (1, -1))
                        new_confidence = predictor.predict(new_enc)
                        updated_probs.append(new_confidence - confidence)
                        if len(self.predictors) > 1:
                            other_predictor = self.predictors[0] if predictor_idx != 0 else self.predictors[1]
                            old_other_confidence = other_predictor.predict(np.reshape(encoding, (1, -1)))
                            new_other_confidence = other_predictor.predict(new_enc)
                    ...
                    print("Mean confidence update for gradient", mean_updated_probs)
                    if len(self.predictors) > 1:
    ...
    def get_disparity_metric(self, predictions, ys, protected_idx=1):
        ...
        return disparity_impact, dem_disparity

    def predict_protected(self, encodings, ys, protected_idx=1):
        ...
        # protected subspace.
        # relevant_subspace = keras.backend.eval(encodings - in_plane_point)
        relevant_subspace = keras.backend.eval(in_plane_point)
        from sklearn.linear_model import LogisticRegression
        regression_model = LogisticRegression()
        train_frac = 0.5
        num_points = relevant_subspace.shape[0]
        x_train = relevant_subspace[:int(train_frac * num_points)]
        y_train = ys[protected_idx][:int(train_frac * num_points)]
        regression_model.fit(x_train, y_train)

        # Use score method to get accuracy of model
        x_test = relevant_subspace[int(train_frac * num_points):]
        y_test = ys[protected_idx][int(train_frac * num_points):]
    ...
    def eval_proto_diffs(self, concept_idx=0):
        ...
        for i, other_protos in enumerate(self.proto_layers):
            ...
            alignments = np.zeros((len(true_diffs), len(true_diffs), other_protos.num_prototypes, other_protos.num_prototypes))
